...
@st.cache_data
def read_data():
    df=pd.read_csv(r'D:\Saksham\Desktop\Streamlit\quarterly_canada_population.csv')
    return df
...
def display_dashboard(start_date,end_date,target):
    ...
    with tab1:
        ...
        with col1:
            intial=df.loc[df['Quarter']==start_date,target].item()
            final=df.loc[df['Quarter']==end_date,target].item()

            percentage_diff=round((final-intial)/intial * 100,2)
            delta=f'{percentage_diff}%'
            st.metric(start_date,value=intial)
...
# st.markdown('Source table can be found [here](https://www150.statcan.gc.ca/t1/tb1/en/tv.action?pid=1710000901)')
...
with st.form('population-form'):
    ...
    with col2:
        ...
        end_quater=st.selectbox('Quarter',options=['Q1','Q2','Q3','Q4'],index=2,key='end_q')
